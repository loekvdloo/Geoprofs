...
export default function Beoordeling({ auth, aanvragen }) {
    const { processing } = useForm()

    const handleAccept = (id) => {
        router.post(`/verlof/beoordeling/${id}/accept`)
    }

    const handleReject = (id) => {
        router.post(`/verlof/beoordeling/${id}/reject`)
    }
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            <div className="max-w-5xl mx-auto p-6">
                    ...
                    <tbody>
                        {aanvragen.map(a => (
                            <tr key={a.aanvraag_id} className="border-t">
                                <td className="p-3">{a.medewerker?.name}</td>
                                ...
                                <td className="p-3 text-center space-x-2">
                                    {a.status === 'pending' && (
                                        <>
                                            <button
                                                onClick={() => handleAccept(a.aanvraag_id)}
                                                disabled={processing}
                                            ...
                                            <button
                                                onClick={() => handleReject(a.aanvraag_id)}
                                                disabled={processing}
                ...
                {aanvragen.length === 0 && (
                    <p className="text-gray-600 mt-4 text-center">Geen verlofaanvragen gevonden.</p>
                )}
            </div>
        </AuthenticatedLayout>
    )
